  ...
  try {
    for (const query of ["banan", "melk", "kylling", "brød"]) {
      const response = await GET(
        new Request(
          `https://localhost/api/foods?provider=mvt&q=${encodeURIComponent(query)}`,
        ),
      );
      expect(response.status).toBe(200);
      const { foods } = (await response.json()) as {
        foods: import("../lib/tracker-core").Food[];
      };
      expect(foods.length).toBeGreaterThan(0);
      expect(
        foods.every((f) => f.source === "Matvaretabellen" && f.unit === "g"),
      ).toBe(true);
      expect(foods.some((f) => f.kcal100 !== null)).toBe(true);
    }
  } finally {
...
test("bundled nutrition preserves source calories and provenance", () => {
  ...
  for (const raw of snapshot.foods) {
    ...
    if (!food) continue;
    count++;
    expect(food.kcal100).toBe(
      raw.calories?.unit === "kcal" ? raw.calories.quantity : null,
    );
...
test("mobile food search uses the bundled library while offline", async ({
  page,
}) => {
  ...
  await page.evaluate(() => {
    (
      window as Window & {
        Capacitor?: Pick<typeof Capacitor, "isNativePlatform">;
      }
    ).Capacitor!.isNativePlatform = () => true;
  ...
  await page
    .getByRole("button", { name: /Søk etter mat|Søk etter matvare/ })
    .click();
  await expect(
    page.getByRole("combobox", { name: "Matmarked", exact: true }),
  ).toHaveValue("no");
  await expect(
    page.getByRole("radio", { name: "Matvaretabellen", exact: true }),
  ).toHaveCount(0);
  await page
  ...
  await expect(
    page.getByText(/Open Food Facts: Du er frakoblet/),
  ).toBeVisible();
});
