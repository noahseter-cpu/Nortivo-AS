...
The subsequent [product-led refinement](docs/ENTERPRISE_REFINEMENT.md) supersedes that homepage ordering and the affected surface descriptions below: Arc now comes first, with a clearly labelled fictional budget explanation that is not an app screen and neither stores nor sends data; the real, captioned app screenshots appear larger on Products. About leads with inspectable work before the supplied biography, Contact explains the enquiry process, and shared navigation and responsive text improve contact access and readability in nb/en. This ordinary, code-led refinement retains the navy identity, Manrope, existing tokens and motion; it does not regenerate the token files or `.impeccable/design.json` or repair unrelated historical documentation. Independent review returned **SHIP**; publication remains pending as recorded in the linked brief.
